...
fn generate_input(input: &str) -> HashMap<String, HashMap<String, u64>> {
    let mut bag_contents: HashMap<String, HashMap<String, u64>> = HashMap::new();
    ...
    let bag_spec_regex = Regex::new(r"(\d+) (.*) bag[s]?").unwrap();
    for line in input.lines() {
        ...
        if bag_no_contents_regex.is_match(line) {
            ...
            bag_contents.insert(bag_type, contents);
            ...
            for inner_bag_spec in captures[2].split(", ") {
                let inner_bag_captures = bag_spec_regex.captures(inner_bag_spec).unwrap();
                let quantity = inner_bag_captures[1].parse::<u64>().unwrap();
                let inner_bag_type = inner_bag_captures[2].to_string();
                contents.insert(inner_bag_type, quantity);
            }
            bag_contents.insert(bag_type, contents);
        } else {
        ...
        target_bag: &str) -> bool
{
    ...
    for (inner_bag, _quantity) in bag_rules.get(current_bag).unwrap() {
        ...
        result |= check_for_inner_bag(bag_rules, inner_bag, target_bag);
    }
    return result;
}
